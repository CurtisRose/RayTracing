...
void raycast() {
  ...
  int pixelIndex = -1;
  double *backgroundColor = (double *)malloc(sizeof(double) * 3);
  ...
  for (row = scene.pixelHeight - 1; row >=0; row--) {
    for (column = 0; column < scene.pixelWidth; column++) {
      pixelIndex++;
      // Get the center of the Pixel i,j
      ...
      if (minT == -1) {
        viewPlane[pixelIndex].red = backgroundColor[0];
        viewPlane[pixelIndex].green = backgroundColor[1];
        viewPlane[pixelIndex].blue = backgroundColor[2];
      }
      // If there was an intersection
        ...
        while (scene.light[lightIndex].color != NULL) {
          ...
          if (sentinel == 2) {
            ...
            // Color that point
            viewPlane[pixelIndex].red   += fAng * fRad * (incidentDiffuse[0] + incidentSpecular[0]);
            viewPlane[pixelIndex].green += fAng * fRad * (incidentDiffuse[1] + incidentSpecular[1]);
            viewPlane[pixelIndex].blue  += fAng * fRad * (incidentDiffuse[2] + incidentSpecular[2]);
          }
